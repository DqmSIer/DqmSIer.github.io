const MaxStatusSearcher = (function() {
		...
		MaxStatusSearcher.prototype.transUp = function(_transCnt, _baseStatusAddr, _puBaseIndex, _puPartnerIndex, _trBaseIndex, _trFormerIndex) {
		...
		MaxStatusSearcher.prototype.powerUp = function(_isLast, _maxStatusInfoAddr, _starCnt, _transCnt, _baseStatusAddr,
							       _puBaseIndex, _puPartnerIndex, _trBaseIndex) {
			...
			for (let i = 0; i < puBaseQueue.length; i++) {
				...
				for (let j = 0; j < puPartnerQueue.length; j++) {
					...
					if (!_isLast) {
						...
						if (nextStatusInfo.stat.gt(maxStatusInfoAddr.stat))
							maxStatusInfoAddr = nextStatusInfo;
						else if (nextStatusInfo.stat.ge(maxStatusInfoAddr.stat) && nextStatusInfo.cost <= maxStatusInfoAddr.cost)
							maxStatusInfoAddr = nextStatusInfo;
					}
				}
			}
		...
		MaxStatusSearcher.prototype.search = function(_baseStatusList) {
			...
			for (let tc = 0; tc < _baseStatusList.length; tc++) {
				...
				for (let sc = 0; sc < 4; sc++)
					maxStatusInfoAddr = this.powerUp(tc == _baseStatusList.length - 1, maxStatusInfoAddr,
									 sc, tc, baseStatusAddr, puBaseIndex, puPartnerIndex, trBaseIndex);
			}
